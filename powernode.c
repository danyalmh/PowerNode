...
#define SECRETCOOKIE "secretcookie"
#define ERLANGNODE "httpdmaster@localhost"
#define LISTENONPORT 8000
...
void request_handler(struct evhttp_request *req, void *arg)
{
        ...
        int uid = -1;
        if(strncmp(evhttp_request_uri(req), "/id/", 6) == 0){
...
void cnode_run()
{
    ...
    if ((fd = erl_connect(ERLANGNODE)) < 0)
        erl_err_quit("erl_connect");

    fprintf(stderr, "Connected to Node \n\r");
        ...
        if (got == ERL_TICK) {
            ...
            if (emsg.type == ERL_REG_SEND) {
                ...
                if(clients[userid]){
                    ...
                    evbuf = evbuffer_new();
                    evbuffer_add(evbuf, (const void*)body, (site_t) body_len);
                    evbuffer_send_reply_chunk(clients[userid], evbuf);
                    evbuffer_free(evbuf);
                }else{
...
int main(int argc, char **argv)
{
    ...
    struct evhttp *httpd;
    event_init();
    httpd = evhttp_start("0.0.0.0", LISTENONPORT);
